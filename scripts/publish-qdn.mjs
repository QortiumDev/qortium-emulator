...
const ENV_PREFIX = 'QORTIUM_EMULATOR';
const DEFAULT_NODE_API_URL = 'http://127.0.0.1:24891';
const DEFAULT_NAME = 'Emulator';
const DEFAULT_IDENTIFIER = 'Emulator';
const DEFAULT_TITLE = 'Emulator';
const DEFAULT_DESCRIPTION = 'Qortium Emulator browses Qortal game metadata and launches ROMs through Home-mediated cross-chain reads.';
...
async function ensureNameRegistered(name, account) {
  ...
  const rawRegisterBytes58 = buildRegisterNameRawBytes58({
    ...
    data: JSON.stringify({
      app: DEFAULT_TITLE,
      purpose: 'Qortium Emulator QDN app',
    }),
  });
